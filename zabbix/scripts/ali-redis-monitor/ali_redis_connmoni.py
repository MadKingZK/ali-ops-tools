...
def main():
    ...
    instance_ids = '['
    instance_infos = get_group_instances(group)
    ...
    for monitor_info in monitor_infos:
        ...
        if current_value < real_threshold:
            ...
            alert_infos.append(
                '【{instance_name}】redis连接数过少: {current_value} < {real_threshold}'.format(instance_name=instance_name, metric=metric,
                                                                       current_value=current_value, real_threshold=real_threshold))
    alert_info_str = '\n'.join(alert_infos)
